...
public class TimerPanel extends JPanel implements PomodoroService.TimerListener {
    ...
    private Color   currentBtnColor = new Color(175, 45, 35);
    private Color   currentBtnHover = new Color(155, 35, 28);
    ...
    private void switchMode(Mode mode, JButton activeBtn){
        ...
        case WORK -> {
            ...
            // ADD the 4th parameter (Dark Red): new Color(120, 25, 18)
            mainFrame.setThemeColor(c, new Color(180, 50, 40), new Color(160, 38, 28), new Color(120, 25, 18));
            
            currentBtnColor = new Color(175, 45, 35);
            currentBtnHover = new Color(155, 35, 28);
            btnStart.setForeground(c);
            syncScrollBg(c);
            if (btnAdd != null) btnAdd.setBackground(currentBtnColor);
            if (footerPanel != null) footerPanel.setBackground(new Color(160, 45, 36));
            yield c;
        }
        case SHORT_BREAK -> {
            Color c = UI.SHORT_GRN;
            activeBtn.setBackground(new Color(38, 100, 72));
            currentBtnColor = new Color(45, 115, 85);
            currentBtnHover = new Color(35, 95, 65);
            
            // ADD the 4th parameter (Dark Green): new Color(38, 100, 72)
            mainFrame.setThemeColor(c, currentBtnColor, currentBtnHover, new Color(38, 100, 72));
            
            btnStart.setForeground(c);
            syncScrollBg(c);
            if (btnAdd != null) btnAdd.setBackground(currentBtnColor);
            if (footerPanel != null) footerPanel.setBackground(new Color(38, 100, 72));
            yield c;
        }
        case LONG_BREAK -> {
            Color c = UI.LONG_BLUE;
            activeBtn.setBackground(new Color(36, 72, 120));
            currentBtnColor = new Color(45, 85, 140);
            currentBtnHover = new Color(35, 70, 115);
            
            // ADD the 4th parameter (Dark Blue): new Color(36, 72, 120)
            mainFrame.setThemeColor(c, currentBtnColor, currentBtnHover, new Color(36, 72, 120));
            
            btnStart.setForeground(c);
            syncScrollBg(c);
            if (btnAdd != null) btnAdd.setBackground(currentBtnColor);
            if (footerPanel != null) footerPanel.setBackground(new Color(36, 72, 120));
            yield c;
        }
    };
    }
    ...
    public void onSessionComplete(Mode completed, Mode next) {
        ...
        // 2. Check user settings to see if the NEXT mode should auto-start
        ...
        // 3. Switch the UI tab to the new mode
        ...
        // 4. Handle Auto-Start vs Manual Start
        if (shouldAutoStart) {
            // Automatically begin the next timer immediately
            service.start();
            btnStart.setText("PAUSE");
        } else {
            // Stop, reset button, and wait for the user
            btnStart.setText("START");
            ...
            // Only show the popup if auto-start is OFF, because popups freeze the timer!
